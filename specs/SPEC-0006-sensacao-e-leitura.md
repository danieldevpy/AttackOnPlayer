...
- **VFX nomeados:** registry data-driven (`muzzle_flash`, `hit_spark`, `death_burst`, `shield_pop`, `flag_aura`, `pickup_glint` + fila inicial do backlog vivo: `speed_up_trail`, `buff_cooldown_ring`, `blood_hit`, `level_up_auto`, `upgrade_chosen_aura`), derivados de eventos já emitidos; pool com orçamento global. **Regra de intensidade (§9-A4):** automático = leve; escolha manual = chamativo estilo "aura". Backlog contínuo em `docs/mechanics/vfx-juice-backlog.md` — o CD adiciona quando sentir necessidade, qualquer leva puxa.
- **Toasts (`toast_text`):** sistema de mensagens de texto com efeito personalizado (fade/slide), fila no canto — nunca no centro da tela; substitui os textos crus do HUD (streak, card escolhido, farm_event). Faz parte do T-023.
