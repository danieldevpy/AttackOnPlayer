...
- **Velocidade de ataque por skill (bugfix pós-teste manual):** antes o bot puxava o gatilho a cada tick que estivesse no alcance, ficando limitado só pelo cooldown do lançador — igual pra `fraco`/`medio`/`forte`, e "impossível de matar" na prática. Agora cada skill tem seu próprio `fireIntervalMs: [min, max]` (`fraco` 1000–1900ms, `medio` 550–1050ms, `forte` 280–600ms); a cada tiro, o próximo intervalo é sorteado dentro da faixa (nunca 100% fixo — é um "peso" aleatório, não uma cadência de metrônomo). O cooldown do lançador (`LAUNCHERS[...].fire.cooldownMs`, o mesmo pra humano e bot) continua valendo por cima — o intervalo da skill só pode segurar o bot, nunca fazer ele atirar mais rápido que a arma permite.
- **Anti-stuck (bugfix pós-teste manual):** bot compara sua posição a cada tick (100ms) com a do tick anterior; se pretende andar (`moveVec` com módulo > 0.3) mas quase não desloca por `STUCK_TICKS_THRESHOLD` ticks seguidos (~500ms), entende que colidiu com um prop/parede/outro player e força um movimento lateral (perpendicular ao vetor pretendido, lado sorteado) por ~350–700ms antes de retomar o comportamento normal. Não usa raycasting nem conhece a geometria do mapa — só observa a posição autoritativa que o servidor já resolve (`moveWithCollision`), então funciona igual contra qualquer obstáculo sem manutenção extra quando o mapa mudar. `BOT_VERBOSE=1` loga `"preso — escapando lateralmente"` quando aciona.
