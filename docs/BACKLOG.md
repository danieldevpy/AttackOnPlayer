...
- **T-046** 〔QA〕 **Smoke de integração da SPEC-0011**: servidor em porta livre (2567/5173 podem estar ocupadas; usar `PORT=2601+`) + 8–12 bots headless por ≥3 min; observar ao vivo `debug_event`s: spawn/pickup de `weapon` (nunca 2, respawn 15–30 s), `flag_cooldown_start`→`flag_respawn` (ciclo completo), `xp_combo` (boosted a partir da 3ª), bots convergindo em alvo de banda alta sem monopólio. **Sem mudar código de jogo** — só rodar, observar, relatar (curto, em `docs/prompts/PROMPT-0039.md` §resultado ou novo relatório em `docs/ai/`). Gates ×3 + tsc ×3 ao final.
- **T-047** 〔docs〕 **Doc de mecânica da bandeira**: nenhum arquivo em `docs/mechanics/` descreve o ciclo completo (acesa → carregada/apagada → drop → 5 s → cooldown 60 s → renasce no centro, XP 2×, assentamento válido). Criar `docs/mechanics/flag.md` (ou seção no doc de loop) em PT-BR, curto, citando as constantes-dial. Docs-only.
