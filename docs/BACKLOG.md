...
**F2.5 — Sobrevivência & recursos de vida (SPEC-0010)** — *gameplay; roda em paralelo/após F3, obrigatório antes do go-live (F6). Decisão do CD pendente na spec.*
- **T-033** 〔M〕 **Recompensa de kill contextual** (ADR-017): no instante do abate, conta inimigos vivos no raio de combate (`COMBAT_THREAT_RADIUS`) do matador. Duelo (0) → bônus de XP; briga (≥1) → cura % da vida **faltante** escalando com nº de inimigos, teto `KILL_HEAL_MISSING_FRAC_MAX`, sem overheal. Server-only, testável com bots isolados vs. aglomerados. Contexto: `specs/SPEC-0010-*.md`, `packages/server/src/rooms/ArenaRoom.ts` (bloco de kill ~L294), `constants.ts`
- **T-034** 〔M〕 **`hp_orb` — orbe de vida escasso**: coletável novo (+`HP_ORB_AMOUNT`, clampa em maxHp). Passe de spawn **dedicado** (fora do orçamento genérico): teto `HP_ORB_MAX`, distâncias mínimas próprias de player (`HP_ORB_MIN_PLAYER_DIST`) e de outro hp_orb (`HP_ORB_MIN_SELF_DIST`), reposição lenta. Placeholder render + coleta. Contexto: SPEC-0010, `spawnCollectible`/`createCollectible` no Room, `constants.ts` · depende: T-033 (compartilha constantes/estender `CollectibleKind`)
- **T-035** 〔G〕 **`shield_temp` — escudo temporário**: coletável novo (máx `SHIELD_TEMP_MAX`=2, espaçamento próprio). Novo `EffectKind` `damage_reduction` no EffectSystem → campo sincronizado `Player.damageTakenMult`; `projectiles.ts` multiplica o dano recebido (reduz, **não** bloqueia — distinto de `blockedByShield`). Tag HUD + VFX de escudo (regra de intensidade). Contexto: SPEC-0010, `effects.ts`, `projectiles.ts`, `ArenaState.ts` · depende: T-034
