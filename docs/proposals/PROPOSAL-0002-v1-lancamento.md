...
**A4 — Juice contínuo com regra de intensidade (adição 2026-07-05, sem reestruturar fases).**
Mais efeitos visuais entram como **backlog vivo** (`docs/mechanics/vfx-juice-backlog.md`) que o CD alimenta quando sente a necessidade jogando — qualquer leva puxa itens como bônus via registry da T-022. **Regra de intensidade:** evento **automático** = efeito leve; **escolha manual do jogador** (card/skill/reroll) = efeito chamativo estilo "aura" — feedback proporcional à agência. Primeiros itens: `speed_up_trail`, `buff_cooldown_ring`, `blood_hit`, `level_up_auto` (leve) vs `upgrade_chosen_aura` (chamativo), e `toast_text` (mensagens com efeito personalizado, não invasivas — entra no T-023/HUD).
