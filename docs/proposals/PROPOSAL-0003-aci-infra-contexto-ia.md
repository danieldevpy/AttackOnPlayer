...
Isto **estende** a ADR-004 (fica coerente): continua sendo processo leve in-repo, agora com uma camada de *automação da busca* que a ADR-004 não tinha. Proponho registrar como **ADR-018** na aprovação (renumerado na integração à evolução — a evolução já tinha um ADR-017 próprio, SPEC-0010).
...
> **Nota de execução (F3):** o "grafo" não é um banco de grafo — é uma função pura que varre as seções já indexadas pela F2 e casa nomes de símbolo (F1) ou docIds (`ADR-NNN`/`SPEC-NNNN`/...) como palavra isolada no texto (`word boundary`, sem falso positivo tipo "Effect" casando dentro de "EffectSystem"). Zero armazenamento novo — computado sob demanda a partir de `code:all`/`docs:all`, coerente com a leveza da F0/F1/F2 (nenhuma dependência nova, nenhum cache extra). O "resumo automático" também não introduz sumarização nova: para spec/prompt/proposal é a seção de nível 1 já extraída pela F2 (título + metadados + intro antes do primeiro `##`); para ADR, a própria seção **é** o resumo, já que o DECISION_LOG é escrito no formato compacto contexto→decisão→consequência. "Resumo de pacote" (packages/\*) não foi implementado — nenhum pacote do jogo tem doc dedicado indexado hoje; reavaliar se/quando existir.

> **Fix pós-adoção (uso real via CLI, antes da F4/F5):** ao testar o hábito recém-instruído no `AGENTS.md`, um agente rodou `aci search "JWT RS256 guest jwks"` e `aci search "guest link auth"` e ambos voltaram vazios — `searchCode`/`searchDocs` tratavam a query inteira como **uma frase literal** (`query.toLowerCase().includes(...)`), então só funcionavam pra 1 palavra ("JWT" sozinho já achava SPEC-0008 com 93% de economia). Corrigido: a query agora é tokenizada por espaço e cada termo vira um OR pontuado (docId/nome > heading/assinatura > snippet), preservando o comportamento de 1 termo de antes (`ADR-014` continua um token só, hífen não quebra o identificador). As duas queries acima agora acham SPEC-0008 com ~91–95% de economia. 3 testes de regressão cobrindo exatamente esse caso.

> **Nota de execução (F5):** antecipada antes da F4 (contexto por feature), a pedido do CD — "quero que ela funcione por MCP para facilitar o uso do agente". F5 não depende de F4 (só de F1-F3, que já existiam), então não há razão pra esperar. Implementação: `src/mcp/handlers.ts` extrai a lógica de `cli.ts` (search/find_symbol/related/summary/stats/index) em funções puras que devolvem dados estruturados em vez de imprimir no terminal (`codeStore`/`docsStore` também extraídos pra `store/stores.ts`, compartilhados pelos dois transportes); `src/mcp/server.ts` embrulha esses handlers em 6 tools MCP (`aci_search`, `aci_find_symbol`, `aci_related`, `aci_summary`, `aci_stats`, `aci_index`) via `@modelcontextprotocol/sdk` (`McpServer` + `StdioServerTransport` — únicas deps novas do pacote, ambas devDependency→dependency porque agora há um processo de runtime, não só build). Registrado em `.mcp.json` na raiz (`npx tsx packages/aci/src/mcp/server.ts`) — Claude Code carrega automaticamente ao abrir o projeto, sem configuração manual. `context` (F4) continua stub — não foi implementada agora, só a porta MCP pras capacidades que já existiam. Validado de ponta a ponta com `Client`/`StdioClientTransport` do próprio SDK spawnando o servidor real e chamando as 6 tools contra o corpus real do repo (mesmas economias de ~91-97% medidas nas notas de execução da F1-F3). `cli.ts` não mudou de comportamento — só passou a importar `codeStore`/`docsStore` do módulo compartilhado. 9 testes novos em `mcp/handlers.test.ts` (cache isolado em dir temporário); suíte completa do pacote: 51/51 verdes.
...
| **F3 — Grafo de relações + resumos** ✅ | arestas doc↔código↔spec↔ADR por menção lexical (nota de execução abaixo); `relatedDocs`/`docsReferencing` + CLI `related`; resumo automático via `summarize` + CLI `summary` (reaproveita a seção de nível 1 da F2, sem heurística nova) | F1, F2 | "quem governa `ProjectileSystem`?" → `related ProjectileSystem` acha ADR-011/013/014 + SPEC-0003..0005, ~90% economia |
...
| **F5 — Servidor MCP + integração agentes** ✅ | `mcp/server.ts` (stdio, nota de execução abaixo); ponteiro no AGENTS.md priorizando MCP sobre CLI; `.mcp.json` na raiz para Claude Code (Codex/Gemini/Cursor configuram apontando pro mesmo comando). (Slot opcional p/ embeddings fica documentado, não implementado.) | F1-F3 | agente externo chama as tools — validado via `Client`/`StdioClientTransport` do SDK MCP, ~91-97% de economia nas mesmas consultas da F1-F3 |
...
Cada fase fecha com o ritual do projeto (DEVLOG + PROMPT-NNNN + atualização de ponteiros). ADR-018 entra junto da F0/aprovação (nota: renumerado de ADR-017 na integração à evolução).
