...
- Backend Django (`backend/`, autenticação/plataforma) é **opcional** — `PLATFORM_ENABLED`
  vem `off` por padrão (`ArenaRoom`), o jogo funciona 100% em modo guest sem ele. Não
  precisa subir Django/Postgres pra esse deploy simples.
...
- Tráfego (posição, chat se houver, token de guest) trafega **sem criptografia** — visível
  pra quem estiver no meio do caminho da rede. Aceitável pra uma partida casual com
  amigos; não é o que se quer pro lançamento público (por isso o SPEC-0009 exige TLS).
- Sem domínio, o IP pode mudar se a VPS for recriada — amigos precisam do IP novo.
- Sem Docker/Django, não há isolamento nem persistência de conta — é só o jogo, guest-only.
...
3. Instala `pm2` se não tiver.
4. `git pull` (se for um clone git).
5. `npm install` na raiz (workspaces).
6. `vite build` do client com `VITE_SERVER_URL=ws://IP:2567` — fixa o endereço certo no bundle.
7. Sobe o game server via pm2 (`aop-server`, porta 2567) e o client estático via
   `pm2 serve` (`aop-client`, porta 5173, `--spa`).
8. Se `-b` foi passado, sobe os bots headless via pm2 (`aop-bots`) — conectam em
...
9. `pm2 save` + imprime o comando de `pm2 startup` (rodar uma vez pra sobreviver a reboot).
10. Libera as portas no `ufw` (avisa pra conferir o firewall do provedor também).
11. Checa `/health` do server e imprime as URLs finais (+ status dos bots).

**Rodar de novo** (pra atualizar depois de um `git push`, ou pra ligar/desligar bots) é o
mesmo comando — o script é idempotente: atualiza código, rebuilda e reinicia os processos
pm2. Rodar sem `-b` depois de já ter rodado com `-b` desliga e remove os bots.

### Comandos úteis pós-deploy

```bash
pm2 status                 # os processos (server, client, bots se ativos), uptime, restarts
...
BACKLOG) — ele é um atalho pra jogar com amigos *antes* disso. Migrar faz sentido quando:

- Quiser divulgar publicamente (link compartilhável sem aviso de "conexão não segura" nem
  depender de decorar um IP).
- Quiser usar o backend Django (contas, stats persistentes, OAuth) de verdade.
- Quiser Docker (ambientes reproduzíveis, rollback por tag de imagem) e hardening
  (rate-limit, backup de Postgres, `/healthz`).
