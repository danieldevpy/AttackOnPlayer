...
| `pytest` (backend Django, T-027+T-028a) — **79 testes** | accounts (modelo/JWT/guest/JWKS/link/**registro/login por email+senha**), maps (validação espelho do TS + `import_maps` + endpoints), gameops (`RoomConfig`/`GameEvent`/config efetiva por janela), telemetry (ingestão batch + schema version), tudo contra Postgres real | Google OAuth (adiado, ADR-020); integração real do Node/client consumindo o Django em produção (coberta manualmente, ver Matriz) |
...
| `authVerifier.test.ts` (server, T-028b) — **6 testes** | Token válido (claims completas), `is_guest`, expirado, issuer errado, malformado e JWKS fora do ar — todos caem pra `null` (guest) sem lançar | Integração real do join do Colyseus com o Django (coberta manualmente, ver Matriz) |
...
| **Auth email+senha + verificação de JWT no join (T-028a/b)** | pytest cobre registro/login (email duplicado, senha fraca, credenciais erradas); `authVerifier.test.ts` cobre token válido/expirado/issuer errado/malformado/JWKS fora do ar | `POST /auth/register` → join com `authToken` (`PLATFORM_ENABLED=1`) → `Player.accountId` setado no servidor; token malformado cai pra guest sem rejeitar o join |
| **Client: janela de login/registro (T-028c)** | — | Pill no canto (nunca modal) → registrar/entrar/logout sem derrubar a partida (HUD segue subindo de nível); erro de credencial aparece no painel; guest local registra no Django em best-effort |
