 ...
 */
export function buildMap(w, h, seed) {
    ...
    const props = [];
    const nearSpawn = (x, z, pw, ph) => spawns.some((s) => Math.abs(s.x - (x + pw / 2)) + Math.abs(s.z - (z + ph / 2)) < SAFE_ZONE_RADIUS);
    // nenhum vizinho (incl. diagonais) já ocupado → props nunca se tocam, logo nunca fecham um corredor.
    const isIsolated = (x, z, pw, ph) => {
    ...
    while (placed < targetProps && attempts < targetProps * 25) {
        ...
        if (nearSpawn(x, z, pw, ph))
            continue;
        if (!isIsolated(x, z, pw, ph))
            continue;
        for (let dx = 0; dx < pw; dx++)
            cells[z * w + (x + dx)] = TILE_WALL;
...
export function zoneAt(map, x, z) {
    let inWar = false;
    for (const zone of map.zones) {
        if (Math.hypot(zone.cx - x, zone.cz - z) > zone.radius)
            continue;
        if (zone.kind === "safe")
            return "safe";
        inWar = true;
...
export function spawnPoints(w, h) {
    const oddMid = (n) => {
...
export function moveWithCollision(map, px, pz, dx, dz, radius) {
    let x = px + dx;
    if (circleHitsWall(map, x, pz, radius))
        x = px;
    let z = pz + dz;
    if (circleHitsWall(map, x, z, radius))
        z = pz;
    return { x, z };
}
export function circleHitsWall(map, cx, cz, r) {
