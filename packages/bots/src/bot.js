...
globalThis.WebSocket = WebSocket; // polyfill p/ colyseus.js em Node
...
function bfsPath(map, fx, fz, tx, tz) {
    ...
    const path = [];
    for (let i = goal; i !== fz * w + fx; i = prev[i])
        path.push({ x: (i % w) + 0.5, z: ((i / w) | 0) + 0.5 });
    return path.reverse();
}
async function runBot(i) {
    ...
    const think = setInterval(() => {
        ...
        let target;
        let best = Infinity;
        state.collectibles?.forEach?.((c, id) => {
