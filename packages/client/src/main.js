...
function buildWorld(w, h, seed) {
    ...
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(w, h), new THREE.MeshLambertMaterial({ color: 0x2e7d32 }));
    ...
    const border = new THREE.InstancedMesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshLambertMaterial({ color: 0x616161 }), perimeter.length);
    ...
    const ZONE_COLOR = { safe: 0x2f6fa8, war: 0x8a2f2f };
    const zoneTiles = { safe: [], war: [] };
    for (let z = 1; z < h - 1; z++) {
        for (let x = 1; x < w - 1; x++) {
            if (isWall(map, x, z))
                continue;
    ...
    Object.keys(zoneTiles).forEach((kind) => {
        const tiles = zoneTiles[kind];
        if (!tiles.length)
            return;
        const mesh = new THREE.InstancedMesh(new THREE.PlaneGeometry(1, 1), new THREE.MeshBasicMaterial({ color: ZONE_COLOR[kind], transparent: true, opacity: 0.45 }), tiles.length);
        tiles.forEach(([x, z], i) => {
...
// ---------- Rede ----------
const url = location.hostname === "localhost" || location.hostname.startsWith("192.")
...
async function connect() {
    try {
        ...
        room.onMessage("pong", (t) => (ping = Math.round(performance.now() - t)));
        room.onMessage("announce", (msg) => {
...
function syncWorld() {
    ...
    if (!st)
        return;
    if (!worldBuilt && st.mapW > 0)
        buildWorld(st.mapW, st.mapH, st.mapSeed);
    if (!worldBuilt)
        return;
    const seenP = new Set();
    st.players.forEach((p, id) => {
    ...
    st.projectiles?.forEach((proj, id) => {
        seenProj.add(id);
        let mesh = projectileMeshes.get(id);
        if (!mesh) {
            mesh = new THREE.Mesh(new THREE.SphereGeometry(0.3), new THREE.MeshBasicMaterial({ color: 0xffaa00 }));
...
function updateHud(now) {
    ...
    st.players.forEach((p, id) => {
        ...
        const fx2 = p.effects ? Array.from(p.effects) : [];
        html += `<div class="row">
